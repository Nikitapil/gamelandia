...
export const BattleshipBoard:FC<BattleshipBoardProps> = ({isEnemy, roomData, secondPlayer, firestore}) => {
  ...
  return (
    <div className='battleship__board'>
        {thisBoard?.cells.map(row => row.map(cell => <BattleshipCell firestore={firestore} secondPlayer={secondPlayer} roomData={roomData} cell={cell} key={cell.id} />))}
    </div>
  )
}
