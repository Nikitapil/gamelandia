...
import { ETetrisColors, L_FIGURE_POSSIBLE_DIRECTIONS } from '../../../constants/tetris';
...
export class LFigureRightModel extends TetrisFigureModel {
    ...
    get nextCells() {
        ...
        const x = this.baseElem!.cell.x;
         return {
            right: [this.board.getCell(y - 1, x), this.board.getCell(y, x + 1), this.board.getCell(y, x + 2)], //done
            left: [this.board.getCell(y+1, x), this.board.getCell(y, x - 1), this.board.getCell(y, x - 2)],
            up: [this.board.getCell(y, x-1), this.board.getCell(y - 1, x), this.board.getCell(y - 2, x)], //done
            down: [this.board.getCell(y, x + 1), this.board.getCell(y + 1, x), this.board.getCell(y + 2, x)] //done
        }
      }
}