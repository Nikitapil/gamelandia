...
export const MatchMatch = () => {
  ...
  return (
    <div className='match container'>
        ...
        <p className='match__description'>Just click on cards and find the same.</p>
        <p className='match__description'> Attempts: <span className='attempts_counter'>{attempts}/25</span></p>
        {isWin && <p className='match__win'>You Win!!! <button className='match__new-game' onClick={newGame}>New Game</button></p>}
        {isLoose && <p className='match__loose'>You Loose <button className='match__new-game' onClick={newGame}>New Game</button></p>}
        <div className='match__cards'>
          {cards.map((card, idx) => {
            return (
              <MatchCard name={card.name} flipped={card.flipped} pic={card.pic} onClick={openCard} key={idx} id={idx} disabled={card.disabled || isLoose} />
            )
          })}
        </div>
    </div>
