...
import { NUMBERS_CELL_SIZE, numberColors, NUMBERS_CELLS_GAP } from '../constants';
...
export const NumbersElem = ({ elem }: NumbersElemProps) => {
  const style = useMemo(() => {
    const top = `${elem.y * NUMBERS_CELL_SIZE + (elem.y + 1) * NUMBERS_CELLS_GAP}px`;

    const left = `${elem.x * NUMBERS_CELL_SIZE + (elem.x + 1) * NUMBERS_CELLS_GAP}px`;
